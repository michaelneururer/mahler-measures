def Z(tau,z):
    w = pari([tau,1])
    return pari((w.ellwp(1/8)-w.ellwp(1/2))/(w.ellwp(1/4)-w.ellwp(1/2)))
def paramXYZ(tau,z):
    w = pari([tau,1])
...
def paramXYZ2(tau,z):
    w = pari([tau,1])
    X = pari( (w.ellwp(z)-w.ellwp(1/2))*(w.ellwp(1/8)-w.ellwp(1/4))^2/((w.ellwp(1/8)-w.ellwp(1/2))*(w.ellwp(z+1/4)-w.ellwp(1/4))*(w.ellwp(z)-w.ellwp(1/4))) )
    Y=pari(-(w.ellwp(z)-w.ellwp(1/4))*(w.ellwp(z+1/4)-w.ellwp(1/4))*(w.ellwp(1/8)-w.ellwp(1/2))/((w.ellwp(1/8)-w.ellwp(1/4))^2*(w.ellwp(1/4)-w.ellwp(1/2))) )
    Z = pari((w.ellwp(1/8)-w.ellwp(1/2))/(w.ellwp(1/4)-w.ellwp(1/2)))
    return X,Y,Z
...
#X,Y,Z evaluated on this half-circle. Perhaps I will add functionality for other cusps... at the moment only ShokurovCycle(-1/2,1/2,n,m,t1,t2) works.
...
#Produces a list of ShokurovCycle values.
    ...
    for t1 in range(1,N):
        for t2 in range(1,N):
            try:
                s = ShokurovCycle(cusp1,cusp2,n,m,t1/N,t2/N)
            except PariError:
...
def CycleValues2(cusp1,cusp2,n,m,N, absolutevalues=False):
#Produces a list of ShokurovCycle values.
    list = []
    absolute = []
    for t2 in range(1,N):
...
def DeningerCycle(phi,psi):
    return [e^(I*phi),e^(I*psi), 1-cos(phi)-cos(psi)+sqrt((1-cos(phi)-cos(psi))^2-1)]
def sigmaBL(X,Y,Z):
    return [-(Y+X*Z)/(X+Y*Z), (Y+X*Z)/(X+Y*Z)*(1+Y*Z)/(X+Y),Z]
def sigma(X,Y,Z):
    return [-(1+X*Y*Z)/(X*Y+Z), (1+X*Y*Z)/(X*Y+Z)*(X+Z)/(1+X*Y),Z]
def sigmaNEW(X,Y,Z):
    return [-(Y+X*Z)/(X+Z*Y), -(Z*Y*(X-1)^2*(Y+Z*X))/(X*(Y+Z)^2*(X+Z*Y)),Z]
def TestF(X,Y,Z):
    return X+1/X+Y+1/Y+Z+1/Z-2

def Cycles(epsilon = 0.05):
    sigmaDCycle = []
    DCycle = []
    phipsiList =[]
    for phi in srange(epsilon,2*pi-epsilon,epsilon):
        if phi<=pi:
            for psi in srange(pi-phi+epsilon,pi+phi-epsilon,epsilon):
                sigmaDCycle.append(sigmaNEW(*DeningerCycle(phi,psi)))
                DCycle.append(DeningerCycle(phi,psi))
                phipsiList.append([phi,psi])
        if phi>pi:
            for psi in srange(phi-pi+epsilon, 3*pi - phi-epsilon, epsilon):
                sigmaDCycle.append(sigmaNEW(*DeningerCycle(phi,psi)))
                DCycle.append(DeningerCycle(phi,psi))
                phipsiList.append([phi,psi])
    return phipsiList,DCycle, sigmaDCycle
